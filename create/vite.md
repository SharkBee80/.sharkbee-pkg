    ...
    rollupOptions: {
      output: {
        ...
        manualChunks(id: string) {
          const start = (name: string) => name[0];
          if (id.includes("node_modules")) {
            ...
            if (name[0] == ".pnpm") {
              return "pnpm" + "/" + start(name[1]); // '/' + name[1];
            }
            return "vendor" + "/" + start(name[0]); // '/' + name[0]
          }
        },
        // 3. 区分业务分包与第三方库分包
        chunkFileNames: (chunkInfo: { facadeModuleId: string | null }) => {
          const facadeModuleId =
            chunkInfo.facadeModuleId ? chunkInfo.facadeModuleId.split("/") : [];
        ...
        assetFileNames: (assetInfo: { name: string | undefined }) => {
          if (assetInfo.name === undefined)
            return "undefined/[name]-[hash][extname]";
